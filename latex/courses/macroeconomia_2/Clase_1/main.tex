...
\begin{importante}{Observaciones}
    ...
    \item $\sigma$ bajo implica que a los hogares no les gusta la sustitución itertemporal, es decir, quieren suavizar el consumo presente. 
...
Consiste en que $\{C_t,C_{t+1},S_t,S_{t+1}\}$ se equilibran dadas las variaciones exogenas de $\{Y_t,Y_{t+1}\}$, se cumple simultaneamente que: 
\begin{enumerate}
    \item Los hogares optimizan, es decir se cumplen las condiciones de optimalidad como: \textbf{ecuacion de euler, restriccion de consumo presente, y restriccion de consumo futuro.}
    \begin{itemize}
        \item $u'(C_t)=\beta (1+r_t)\; u'(C_{t+1})$
        \item $C_t+S_t=Y_t$ siempre que $S_{t-1}=0$
        \item $C_{t+1}=Y_{t+1}+(1+r_t)S_t$
    \end{itemize}
    \item El mercado de fondos prestables esta en equilibrio
    $$S_t=0 \iff Y_t-C_t=0 \iff Y_t=C_t$$
\end{enumerate}
\begin{ejemplo}{Logaritmica}
    Sea $u(c)=log(c)$\\
    ...
    \begin{itemize}
        ...
        \item $C_t=Y_t$
    \end{itemize}
    Ahora, sabemos que $$C_t=\frac{1}{1+\beta}(Y_t+\frac{Y_{t+1}}{1+r_t})$$
    Determine $\{C_t,C_{t+1},S_t,S_{t+1}\}$. \\
    Sabemos que $S_t=0$\\
    Si tenemos: $$S_t=(\frac{1}{1+\beta})(\beta Y_t-\frac{Y_{t+1}}{1+r_t})$$ 
    Sabemos que $$1+r_t=\frac{Y_{t+1}}{\beta Y_t}$$
    Si inyectamos este $1+r_t$ en nuestra funcion $C_t=\frac{1}{1+\beta}(Y_t+\frac{Y_{t+1}}{1+r_t})$ llegamos al resultado: $$C_t=Y_t$$
\end{ejemplo}
\begin{comentarioprofe}{Observe}
    Considere: $$1+r_t=\frac{Y_{t+1}}{\beta Y_t}$$
    \begin{itemize}
        \item Si $\uparrow Y_{t+1}$ en eq. general $\uparrow r_t \Rightarrow{} C_t$ no cambia. 
        \textit{El hogar quiere ahorrar menos/endeudarse mas a $r_t$}
        \item Sea $r_0<r_1$: 
        \begin{itemize}
            \item En $r_0$ hay exceso de demanda de fondos prestables. 
            Esto genera un alza en $r_t$ hasta eliminar el exceso. 
        \end{itemize}
        \item Si $\uparrow\beta \Rightarrow{} \downarrow r_t$
        \begin{itemize}
            \item Mas pacientes, el hogar quiere ahorrar mas. 
            \item exceso de oferta de fondos prestables 
            \item $\downarrow r_t$
        \end{itemize}
    \end{itemize}
\end{comentarioprofe}
\begin{ejemplo}{Logaritmica}
    ...
    Tal que: $$C_t=(\frac{1+r_t}{2+r_t})(Y_t+\frac{Y_{t+1}}{1+r_t})$$
    Sabemos por la igualdad que entonces: $\beta (1+r_t)=1\; \Rightarrow{} 1+r_t=\frac{1}{\beta}$\\
    Si $u(c)=log(c)$ sabemos que: $$1+r_t=\frac{Y_{t+1}}{\beta Y_t}$$
    Se veria de forma: 
    \begin{figure} [H]
        \centering
        \includesvg[width=0.5\linewidth]{figuras/clase_1/macro_5.drawio.svg}
        \label{fig:placeholder}
    \end{figure}
\end{ejemplo}
\subsection{Restricciones de Endeudamiento}
Recuerde: 
\begin{align*}
    \underbrace{max}_{\{ C_t, C_{t+1},S_t,S_{t+1}\}} u(C_t)+\beta\cdot u(C_{t+1}) \\ \text{s.a} \\ C_t+S_t & =Y_t \\  C_{t+1}+S_{t+1} & =Y_{t+1}+(1+r_t)S_t 
\end{align*}
...
Por ende tendriamos: 
\begin{align*}
    \underbrace{max}_{\{ C_t, C_{t+1},S_t,S_{t+1}\}} u(C_t)+\beta\cdot u(C_{t+1}) \\ \text{s.a} \\ C_t & =Y_t+d_t \\  C_{t+1}+(1+r_t)d_t & =Y_{t+1} \\
    d_t&<k \cdot Y_t\; ;\; 0 \leq k 
\end{align*}
Visto graficamente tendriamos: 
\begin{figure} [H]
        \centering
        \includesvg[width=0.5\linewidth]{figuras/clase_1/macro_6.drawio.svg}
        \label{fig:placeholder}
    \end{figure}
\begin{ejemplo}{Logaritmica}
    ...
    \begin{align*}
        d_t^R=&k\cdot Y_t \\ C_t^R -Y_t=&k\cdot Y_t\\ C_t^R=&(1+k)Y_t\; \textit{En este caso NO se cumple la ecuacion de Euler.}\\
    \end{align*}
    Note que si $K=0$, no se pueden endeudar del todo.\\
    Y si la restriccion es vinculante $C_t^R=Y_t$
\end{ejemplo}
\begin{comentarioprofe}{Movimiento del K}
Si tenemos $C_t^R=(1+k)Y_t$ sabemos que:
\begin{itemize}
    \item Si el hogar es \textbf{Hand-to-mouth} las fricciones finacieras impiden acceder plenamente al mercad de fondos prestables. 
    \item Si no hubuesen fricciones el consumo seria $C_t=(\frac{1}{1+\beta})(Y_t+\frac{Y_{t+1}}{1+r_t})$, en este caso la PMC es mas alta que en hogares \textbf{H2M}
\end{itemize}
\end{comentarioprofe}
\subsection{Consumo Intertemporal}
Recordemos:
\begin{align*}
    \underset{\{C_{t+j},Y_j\}}{max}\; \sum_{j=0}^T \beta^ju(C_{t+j})\\
    s.a \\
    \sum^T_{j=0}\frac{C_{t+j}}{(1+r)^j}=\sum^T_{j=0}\frac{Y_{t+j}}{(1+r)^j}
\end{align*}
Y recordemos que $\beta(1+r_t)=1$ al igual que $\bar{C}=C_{t+j}\; \forall\; j=1,...,n$\\
...
\begin{figure} [H]
        \centering
        \includesvg[width=0.5\linewidth]{figuras/clase_1/macro_7.drawio.svg}
        \label{fig:placeholder}
\end{figure}

Suponga para $Y_t>0$ dado tenemos: 
\begin{align*}
    Y_{t+1}=&(1+g)Y_t\\ Y_{t+2}=&(1+g)Y_{t+1}=(1+g)^2Y_t\\
    \vdots \\Y_{t+3}=&(1+g)^3Y_t\\\vdots \\ Y_{t+R}=&(1+g)^j\cdot Y_t     
\end{align*}
De forma que: 
\begin{figure} [H]
        \centering
        \includesvg[width=0.5\linewidth]{figuras/clase_1/macro_8.drawio.svg}
        \label{fig:placeholder}
\end{figure}
...
\newpage
\subsection{Cambios en Variables Exogenas}
Anteriormente demostramos que:
$$\bar{C}=(\frac{1-\beta}{1-\beta^{T+1}})(\sum_{j=0}^T\beta^jY_{t+j})$$

\begin{figure} [H]
        \centering
        \includesvg[width=0.5\linewidth]{figuras/clase_1/macro_9.drawio.svg}
        \label{fig:placeholder}
\end{figure}
\begin{align*}
    \bar{C}=(\frac{1-\beta}{1-\beta^{T+1}})(\sum_{j=0}^R\beta^jY_{t+j}+\sum_{j=R+1}^T\beta^jY_{t+j})\\
    =(\frac{1-\beta}{1-\beta^{T+1}})(\sum_{j=0}^R\beta^j\mathbf{(1+g)^j\cdot Y_t}+\sum_{j=R+1}^T\beta^j\mathbf{Y_R})\\
    =(\frac{1-\beta}{1-\beta^{T+1}})(Y_t \cdot\sum_{j=0}^R\beta^j\mathbf{(1+g)^j\cdot }+Y_R\sum_{j=R+1}^T\beta^j)\\
    \vdots\\
    \bar{C}=(\frac{1-\beta}{1-\beta^{T+1}})(Y_t \cdot \frac{1-(\beta(1+g))^{R+1}}{1-\beta(1+g)} +Y_R \frac{1-\beta^{T-R}}{1-\beta}\beta^{R+1})
\end{align*}
Sabiendo que $\bar{C}=C_t=C_{t+j}$ sabemos que: 
\begin{itemize}
    \item $\bar{C}$ es creciente en $Y_R$
    \item $\bar{C}$ es creciente en $Y_t$
    \item $\bar{C}$ es creciente en $g$
    \item $\bar{C}$ es creciente en $R$, con un $Y_R$
\end{itemize}
